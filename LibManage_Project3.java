class LibManage_Project3
{
    ...
    public static int date_number_finder(String date) { //return the date number which is the value to be used in calculations of dates
        ...
        date2[1] = to_int(date.substring(first_index+1, date.indexOf("/", first_index+1)));
    ...
    public static String return_date_finder(String Issue_date) {
        int days_month[]={31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
        int first_index = Issue_date.indexOf("/");
        int dd=to_int(Issue_date.substring(0, first_index));
        int mm=to_int(Issue_date.substring(first_index+1, Issue_date.indexOf("/", first_index+1)));
        int yy=to_int(Issue_date.substring(Issue_date.indexOf("/", first_index+1) + 1, Issue_date.length()));
        String return_date="";
        if(dd+7 > days_month[mm-1]) {
            //make return date the same year, but month +1 and days the reminader
            return_date+=(dd+7-days_month[mm-1]) + "/" + (mm+1) + "/" + yy;
        }
        else
        {
            return_date+=dd+7 + "/" + mm + "/" + yy;
        }
        return return_date;
    }
}//class
